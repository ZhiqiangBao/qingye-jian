...
<body>
  ...
  async function openTemplateModal() {
    ...
    try {
      ...
      const data = await res.json();
      if (!res.ok) throw new Error(data.error || `HTTP ${res.status}`);
      templateHint.textContent = `模板目录：${data.folder || "templates"} · 当前会导入到：${
      ...
      if (!list.length) {
        templateGrid.innerHTML =
          `<div class="template-empty">templates 里还没有模板。把 .md 放进去后再打开本面板。</div>`;
  ...
  async function importTemplate() {
    ...
    // Import into the folder currently being browsed (default: workspace root = document/)
  ...
  document.getElementById("btnSkinLoadExample")?.addEventListener("click", async () => {
    try {
      ...
      const data = await res.json();
      if (!res.ok) throw new Error(data.error || `HTTP ${res.status}`);
      // Strip server-only fields for a clean editable template
      const clean = {
  ...
  (async () => {
    ...
    try {
      const res = await fetch("/api/workspace", { cache: "no-store" });
      const data = await res.json();
      if (res.ok) workspaceRoot = data.workspace || "";
    } catch {}
    // Default workspace is APP/document — open its root directly.
    await refreshList("", true);
  })();
})();
